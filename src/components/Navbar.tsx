...
import {
  Monitor, ShoppingCart, User, Search, Heart,
  Cpu, Layers, HardDrive, MemoryStick, Zap, Box, Wind, Gamepad2, Laptop, ChevronDown, Sun, Moon, X, Menu
} from "lucide-react";
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleCart = () => {
      ...
      if (c) { try { setCartCount(JSON.parse(c).reduce((s: number, i: { qty: number }) => s + i.qty, 0)); } catch { setCartCount(0); } }
      else setCartCount(0);
    };
    const handleWishlist = () => {
      const w = localStorage.getItem("wishlist");
      if (w) { try { setWishlistCount(JSON.parse(w).length); } catch { setWishlistCount(0); } }
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "glass-strong shadow-lg shadow-black/30" : "bg-transparent"}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-14 md:h-16">
        <Link href="/" className="flex items-center gap-2 group shrink-0">
          <div className="w-9 h-9 md:w-10 md:h-10 rounded-lg bg-gradient-to-br from-nexus-blue to-nexus-purple flex items-center justify-center group-hover:shadow-lg group-hover:shadow-nexus-blue/20 transition-all">
        ...
        <div className="flex items-center gap-1 md:gap-3">
          ...
          <button onClick={() => setMobileSearchOpen(true)} className="md:hidden p-2 hover:bg-nexus-surface rounded-lg transition-colors" aria-label="Search">
            <Search className="w-5 h-5" />
          </button>
          <Link href="/wishlist" className="hidden md:flex relative p-2 hover:bg-nexus-surface rounded-lg transition-colors text-nexus-blue">
            <Heart className="w-5 h-5" />
            {wishlistCount > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-nexus-red text-white text-[10px] font-bold rounded-full flex items-center justify-center">{wishlistCount}</span>
            )}
          </Link>
          <Link href="/cart" className="relative p-2 hover:bg-nexus-surface rounded-lg transition-colors">
            <ShoppingCart className="w-5 h-5" />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-nexus-blue text-black text-[10px] font-bold rounded-full flex items-center justify-center">{cartCount}</span>
            )}
          </Link>
          {user ? (
            <Link href="/profile" className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-nexus-surface border border-nexus-border rounded-lg hover:border-nexus-blue/30 transition-colors">
              <User className="w-4 h-4 text-nexus-blue" /><span className="text-sm">{user.name.split(" ")[0]}</span>
            </Link>
          ) : (
            <Link href="/login" className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-nexus-surface border border-nexus-border rounded-lg hover:border-nexus-blue/30 transition-colors">
              <User className="w-4 h-4" /><span className="text-sm">Sign In</span>
            </Link>
          )}
        </div>
      </div>
          ...
          <div className="relative">
            ...
            <AnimatePresence>
              {catOpen && (
                ...
                >
                  {categories.map(c => (
                    <Link key={c.slug} href={`/products?category=${c.slug}`} onClick={() => setCatOpen(false)}
      ...
      <AnimatePresence>
        {mobileSearchOpen && (
          <motion.div
            ...
            className="md:hidden fixed inset-0 z-[100] flex flex-col overflow-y-auto"
            style={{ background: "rgba(9,8,7,0.97)", backdropFilter: "blur(20px)" }}
          >
            <div className="flex items-center gap-3 p-4 mt-2">
              <form onSubmit={handleMobileSearch} className="flex-1 flex items-center gap-2 bg-nexus-surface border border-nexus-border rounded-xl px-4 py-3">
                <Search className="w-5 h-5 text-nexus-muted shrink-0" />
                <input
                  autoFocus
                  type="text"
                  value={mobileSearch}
              ...
              <button onClick={() => setMobileSearchOpen(false)} className="p-2 text-nexus-muted hover:text-nexus-text">
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="px-4 py-2">
              <p className="text-xs text-nexus-muted mb-3 uppercase tracking-wider">Browse by Category</p>
              <div className="grid grid-cols-2 gap-2">
                {categories.map(c => (
                  <Link
                    ...
                    className="flex items-center gap-3 p-3 bg-nexus-card border border-nexus-border rounded-xl hover:border-nexus-blue/50 transition-colors"
                  >
                    <span className="p-1.5 bg-nexus-surface rounded-lg">
                      <c.icon className="w-4 h-4 text-nexus-blue" />
                    </span>
