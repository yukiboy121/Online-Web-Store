...
import { Monitor, User, Mail, Lock, ArrowRight, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
        ...
        <p className="text-nexus-muted mt-2">Join NEXUS PC for the best deals</p>
      </motion.div>
      ...
      >
        {error && <div className="p-3 bg-red-900/20 border border-red-900/50 rounded-lg text-red-400 text-sm flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> {error}</div>}
        <div>
        ...
        <p className="text-center text-sm text-nexus-muted">Already have an account?{" "}<Link href="/login" className="text-nexus-blue hover:underline">Sign In</Link></p>
      </motion.form>
    </div>
  );
}
