...
import { db } from "@/db";
import { products, inventoryLog, categories } from "@/db/schema";
...
export async function GET() {
  try {
    ...
    }).from(products).orderBy(products.stock);
    ...
    return NextResponse.json({ products: allProducts, lowStock, outOfStock, logs, categories: allCategories });
  } catch (e) {
    console.error(e);
